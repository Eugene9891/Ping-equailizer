...
DIRECTION=3
# Speed
SPEED_DOWNLOAD="1024kbit"
SPEED_UPLOAD="1024kbit"
...
function show_usage {
    ...
    echo "Version: $VERSION | Original author/link: ole1986 https://gist.github.com/ole1986/d9d6be5218affd41796610a35e3b069c"
    echo
    echo "Usage: $1 [-r|--remove] [-i|--incoming] [-o|--outgoing] [-d|--delay=] [-I|--index=] [--uspeed=] [--dspeed=] <IP>"
    echo
    echo "Arguments:"
    ...
    echo "  -I|--index=1    : define the index of ip in rules, needed for multiply ips (default: 1)"
    ...
    echo
    echo "Changelog:"
    echo "v1.0.3 - multiply ips supported by providing different id"
...
function tc_outgoing {
    ...
    if [[ $(tc class show dev $INTERFACE |grep '^class htb 1:${INDEX}') ]]; then
        TCC="tc class change"
    else
        # Add classes per ip
    ...
    ##if [ $SPEED_UPLOAD -gt 0 ]; then
    echo "- upload speed $SPEED_UPLOAD"
    $TCC dev $INTERFACE parent 1: classid 1:${INDEX} htb rate $SPEED_UPLOAD
    ##fi
    
    if [ $DELAY -gt 0 ]; then
        TCN="tc qdisc add"
        [[ $(tc qdisc |grep '^qdisc netem ${INDEX}0:') ]] && TCN="tc qdisc change"
        echo "- latency ${DELAY}ms"
        $TCN dev $INTERFACE parent 1:${INDEX} handle ${INDEX}0: netem delay ${DELAY}ms
    fi
    ...
    ##fi
    $TCF dev $INTERFACE protocol ip parent 1: prio $INDEX u32 match ip dst $ADDRESS flowid 1:$INDEX
}
...
function tc_incoming {
    ...
    if [[ $(tc class show dev $VIRTUAL |grep '^class htb 2:${INDEX}') ]]; then
        TCC="tc class change"
    else
    ...
    ##if [ $SPEED_DOWNLOAD -gt 0 ]; then
    echo "- download speed $SPEED_DOWNLOAD"
    $TCC dev $VIRTUAL parent 2: classid 2:${INDEX} htb rate $SPEED_DOWNLOAD
    ##fi
    ...
    if [ $DELAY -gt 0 ]; then
        ...
        echo "- latency ${DELAY}ms"
        $TCN dev $VIRTUAL parent 2:${INDEX} handle ${INDEX}0: netem delay ${DELAY}ms
    fi

    echo "- filter on IP $ADDRESS"
    TCF="tc filter add"
    ##if [[ $(tc filter show dev $VIRTUAL | grep '^filter parent 2: protocol ip pref 1') ]]; then
    ##    TCF="tc filter change"
    ##fi

    $TCF dev $VIRTUAL protocol ip parent 2: prio $INDEX u32 match ip src $ADDRESS flowid 2:$INDEX
}
...
[ $DIRECTION -eq 0 ] && remove_tc && exit 0
